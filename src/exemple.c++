...
void build_select_list() {
  ...
  for (listnum = 0; listnum < 5; listnum++) {
    if (connectlist[listnum] != 0) {
      ...
      if (connectlist[listnum] > highsock) highsock = connectlist[listnum];
    }
  }
}
...
void read_socks() {
  ...
  if (FD_ISSET(sock, &socks)) handle_new_connection();
  ...
  for (listnum = 0; listnum < 5; listnum++) {
    if (FD_ISSET(connectlist[listnum], &socks)) deal_with_data(listnum);
